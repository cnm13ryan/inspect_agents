...
def recency_weight(published_at: datetime | None, now: datetime, *, half_life_days: float = 365.0) -> float:
    ...
    n = _naive(now)
    days = max(0.0, (n - p).total_seconds() / 86400.0)
    if half_life_days <= 0:
        return 1.0
    # 0.5 ** (days / half_life)
...
def topical_similarity(query: str, text: str) -> float:
    """Cheap Jaccard similarity on alnum tokens (lowercased)."""
...
_DOI_RE = re.compile(r"\bdoi:\s*10\.\d{4,9}/\S+\b", re.IGNORECASE)
_ARXIV_ID_RE = re.compile(r"\barxiv:?\s*\d{4}\.\d{4,5}(?:v\d+)?\b", re.IGNORECASE)
_BRACKETED_CITE_RE = re.compile(r"\[\d{1,3}\]")


def citation_present(*, title: str, snippet: str) -> float:
    """Detect common citation indicators in title/snippet.

    Returns a float in [0, 1]:
      - 1.0 if a DOI is present
      - 0.8 if an arXiv id or arxiv.org URL is present
      - 0.5 if any bracketed numeric citation like "[12]" appears
      - 0.0 otherwise
    """
