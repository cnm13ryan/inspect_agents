"""
Simple Architecture — Minimal Demo
...
- Supervisor via `build_supervisor(...)` with built‑in Todo/FS tools and
...
Usage
  uv run python examples/demos/simple_arch/minimal.py "What is x?"
...
def _ensure_repo_paths() -> None:
    ...
    repo = here.parents[3]  # examples/demos/simple_arch/ -> repo root
...
async def main() -> None:
    # Args
    ap = argparse.ArgumentParser(description="Simple Architecture minimal demo")
