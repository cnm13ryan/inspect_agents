...
def test_edit_file_wrapper_accepts_expected_count_and_dry_run(monkeypatch):
    ...
    from inspect_agents.tools_files import files_tool, FilesParams, ReadParams
    ...
    e = edit_file()
    asyncio.run(e(file_path="w.txt", old_string="foo", new_string="bar", replace_all=True, expected_count=2, dry_run=True, instance="wrap"))

    # Apply with expected_count=2
    asyncio.run(e(file_path="w.txt", old_string="foo", new_string="bar", replace_all=True, expected_count=2, instance="wrap"))

    # Verify mutated via unified reader
    f = files_tool()
    out = asyncio.run(f(params=FilesParams(root=ReadParams(command="read", file_path="w.txt", offset=0, limit=10, instance="wrap"))))
