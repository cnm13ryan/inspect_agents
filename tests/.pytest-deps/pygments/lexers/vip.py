...
class VisualPrologLexer(VisualPrologBaseLexer):
    ...
    def analyse_text(self):
        ...
        if re.search(r'^\s*(end\s+(interface|class|implement)|(clauses|predicates|domains|facts|constants|properties)\s*$)', self):
            return 0.98
        else:
            return 0
...
class VisualPrologGrammarLexer(VisualPrologBaseLexer):
    ...
    def analyse_text(self):
        ...
        if re.search(r'^\s*(end\s+grammar|(nonterminals|startsymbols|terminals|rules|precedence)\s*$)', self):
            return 0.98
        else:
            return 0
