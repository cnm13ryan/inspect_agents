...
class RstLexer(RegexLexer):
    ...
    def analyse_text(self):
        if self[:2] == '..' and self[2:3] != '.':
            return 0.3
        p1 = self.find("\n")
        p2 = self.find("\n", p1 + 1)
        if (p2 > -1 and              # has two lines
                p1 * 2 + 1 == p2 and     # they are the same length
                self[p1+1] in '-=' and   # the next line both starts and ends with
                self[p1+1] == self[p2-1]):  # ...a sufficiently high header
            return 0.5
...
class GroffLexer(RegexLexer):
    ...
    def analyse_text(self):
        ...
        if self[:4] == '.TH ':
            return True
        if self[1:3].isalnum() and self[3].isspace():
            return 0.9
...
class WikitextLexer(RegexLexer):
    ...
    def nowiki_tag_rules(self):
        return [
            (rf'(?i)(</)({self})(\s*)(>)', bygroups(Punctuation,
             Name.Tag, Whitespace, Punctuation), '#pop'),
            include('entity'),
            include('text'),
        ]

    def plaintext_tag_rules(self):
        return [
            (rf'(?si)(.*?)(</)({self})(\s*)(>)', bygroups(Text,
             Punctuation, Name.Tag, Whitespace, Punctuation), '#pop'),
        ]

    def delegate_tag_rules(self, lexer, **lexer_kwargs):
        return [
            (rf'(?i)(</)({self})(\s*)(>)', bygroups(Punctuation,
             Name.Tag, Whitespace, Punctuation), '#pop'),
            (rf'(?si).+?(?=</{self}\s*>)', using(lexer, **lexer_kwargs)),
        ]
