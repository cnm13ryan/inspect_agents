...
class BashLexer(RegexLexer):
    ...
    def analyse_text(self):
        if shebang_matches(self, r'(ba|z|)sh'):
            return 1
        if self.startswith('$ '):
            return 0.2
...
class BatchLexer(RegexLexer):
    ...
    def _make_begin_state(self, _core_token=_core_token,
        ...
        if self:
            ...
            suffix = '/compound'
        return [
            ((r'\)', Punctuation, '#pop') if self else
            ...
            (r'@+', Punctuation),
            (rf'((?:for|if|rem)(?:(?=(?:\^[{_nl}]?)?/)|(?:(?!\^)|'
             rf'(?<=m))(?:(?=\()|{_token_terminator})))({_space}?{_core_token_compound if self else _core_token}?(?:\^[{_nl}]?)?/(?:\^[{_nl}]?)?\?)',
            ...
            (rf'rem(((?=\()|{_token_terminator}){_space}?{_stoken}?.*|{_keyword_terminator}{rest_of_line_compound if self else rest_of_line})',
    ...
    def _make_follow_state(self, _label=_label,
                           _label_compound=_label_compound, _nl=_nl,
                           _space=_space, _start_label=_start_label,
                           _token=_token, _token_compound=_token_compound,
                           _ws=_ws):
        suffix = '/compound' if self else ''
        state = []
        if self:
            state.append((r'(?=\))', Text, '#pop'))
        state += [
            (rf'{_start_label}([{_ws}]*)({_label_compound if self else _label})(.*)',
    ...
    def _make_arithmetic_state(self, _nl=_nl, _punct=_punct,
    ...
    def _make_call_state(self, _label=_label,
                         _label_compound=_label_compound):
        state = []
        if self:
            state.append((r'(?=\))', Text, '#pop'))
        state.append((r'(:?)(%s)' % (_label_compound if self else _label),
                      bygroups(Punctuation, Name.Label), '#pop'))
        return state

    def _make_label_state(self, _label=_label,
                          _label_compound=_label_compound, _nl=_nl,
                          _punct=_punct, _string=_string, _variable=_variable):
        state = []
        if self:
            state.append((r'(?=\))', Text, '#pop'))
        state.append((r'({}?)((?:{}|{}|\^[{}]?{}|[^"%^{}{}{}])*)'.format(_label_compound if self else _label, _string,
                       _variable, _nl, r'[^)]' if self else r'[\w\W]', _nl,
                       _punct, r')' if self else ''),
                      bygroups(Name.Label, Comment.Single), '#pop'))
        return state

    def _make_redirect_state(self,
                             _core_token_compound=_core_token_compound,
        ...
        return [
            ...
            (rf'((?:(?<=[{_nlws}])(?<!\^[{_nl}])\d)?)(>>?|<)({_space}?{stoken_compound if self else _stoken})',
...
class ExeclineLexer(RegexLexer):
    ...
    def analyse_text(self):
        if shebang_matches(self, r'execlineb'):
            return 1
