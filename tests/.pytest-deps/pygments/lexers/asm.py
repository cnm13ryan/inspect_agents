...
class GasLexer(RegexLexer):
    ...
    def analyse_text(self):
        if re.search(r'^\.(text|data|section)', self, re.M):
            return True
        elif re.search(r'^\.\w+', self, re.M):
            return 0.1
...
class NasmLexer(RegexLexer):
    ...
    def analyse_text(self):
        # Probably TASM
        if re.match(r'PROC', self, re.IGNORECASE):
            return False
...
class Dasm16Lexer(RegexLexer):
    ...
    def guess_identifier(self, match):
        ident = match.group(0)
