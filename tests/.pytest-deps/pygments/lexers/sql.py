...
class TransactSqlLexer(RegexLexer):
    ...
    def analyse_text(self):
        rating = 0
        if tsql_declare_re.search(self):
            # Found T-SQL variable declaration.
            rating = 1.0
        else:
            name_between_backtick_count = len(
                name_between_backtick_re.findall(self))
            name_between_bracket_count = len(
                name_between_bracket_re.findall(self))
            ...
            if dialect_name_count >= 1 and \
               name_between_bracket_count >= 2 * name_between_backtick_count:
                ...
                rating += 0.1
            if tsql_variable_re.search(self) is not None:
                rating += 0.1
            if tsql_go_re.search(self) is not None:
                rating += 0.1
        return rating
...
class GoogleSqlLexer(RegexLexer):
    ...
    def analyse_text(self):
        tokens = collections.Counter(self.split())
