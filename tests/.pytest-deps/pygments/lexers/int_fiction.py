...
class Tads3Lexer(RegexLexer):
    ...
    def _make_string_state(self, double, verbatim=None, _escape=_escape):
        ...
        escaped_quotes = rf'+|{char}(?!{char}{{2}})' if self else r''
        prefix = '{}{}'.format('t' if self else '', 'd' if double else 's')
    ...
    def _make_tag_state(self, double, _escape=_escape):
        ...
        state_name = '{}{}qt'.format('t' if self else '', 'd' if double else 's')
        token = String.Double if double else String.Single
        escaped_quotes = rf'+|{char}(?!{char}{{2}})' if self else r''
        return [
    ...
    def _make_attribute_value_state(self, host_triple, host_double,
                                    _escape=_escape):
        token = (String.Double if self == r'"' else
                 String.Single if self == r"'" else String.Other)
        ...
        return [
            ...
            (r'{}{}'.format(r'' if token is String.Other else r'\\?', self),
             token, '#pop'),
            include('s/verbatim'),
            include('s/entity'),
