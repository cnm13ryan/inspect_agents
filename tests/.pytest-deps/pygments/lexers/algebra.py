...
class GAPLexer(RegexLexer):
    ...
    def analyse_text(self):
        ...
        if re.search(
            ...
            r"|GlobalFunction|Synonym|SynonymAttr|Property))", self
        ):
            score += 0.7
        ...
        if re.search(
            ...
            r"ImmediateMethod|OtherMethod)|New(Family|Type)|Objectify)", self
        ):
            score += 0.7
...
class MathematicaLexer(RegexLexer):
    ...
    def _multi_escape(self):
        return '({})'.format('|'.join(re.escape(entry) for entry in self))
