...
class DesktopLexer(RegexLexer):
    ...
    def analyse_text(self):
        if self.startswith("[Desktop Entry]"):
            return 1.0
        if re.search(r"^\[Desktop Entry\][ \t]*$", self[:500], re.MULTILINE) is not None:
            return 0.9
        return 0.0
...
class SystemdLexer(RegexLexer):
    ...
    def analyse_text(self):
        if self.startswith("[Unit]"):
            return 1.0
        if re.search(r"^\[Unit\][ \t]*$", self[:500], re.MULTILINE) is not None:
            return 0.9
        return 0.0
...
class SingularityLexer(RegexLexer):
    ...
    def analyse_text(self):
        ...
        result = 0
        if re.search(r'\b(?:osversion|includecmd|mirrorurl)\b', self, re.IGNORECASE):
            result += 0.5
