...
class PerlLexer(RegexLexer):
    ...
    def analyse_text(self):
        if shebang_matches(self, r'perl'):
            return True

        result = 0

        if re.search(r'(?:my|our)\s+[$@%(]', self):
            result += 0.9
...
class Perl6Lexer(ExtendedRegexLexer):
    ...
    def _build_word_match(self, boundary_regex_fragment=None, prefix='', suffix=''):
        if boundary_regex_fragment is None:
            return r'\b(' + prefix + r'|'.join(re.escape(x) for x in self) + \
                suffix + r')\b'
        else:
            return r'(?<!' + boundary_regex_fragment + r')' + prefix + r'(' + \
                r'|'.join(re.escape(x) for x in self) + r')' + suffix + r'(?!' + \
                boundary_regex_fragment + r')'

    def brackets_callback(self):
        def callback(lexer, match, context):
            ...
            yield match.start(), self, text[match.start():end_pos + n_chars]
            context.pos = end_pos + n_chars

        return callback

    def opening_brace_callback(self, match, context):
        stack = context.stack
    ...
    def closing_brace_callback(self, match, context):
        stack = context.stack
    ...
    def embedded_perl6_callback(self, match, context):
        context.perl6_token_nesting_level = 1
        yield match.start(), Text, context.text[match.start():match.end()]
        context.pos = match.end()
    ...
    def analyse_text(self):
        ...
        if shebang_matches(self, r'perl6|rakudo|niecza|pugs'):
            return True
