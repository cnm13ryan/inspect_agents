...
if TYPE_CHECKING:
    from execnet.gateway_base import Channel, ExecModel
...
try:
    ...
    from __main__ import (
        Message,  # type: ignore[no-redef]
        Popen2IO,  # type: ignore[no-redef]
    )
...
class Popen2IOMaster(Popen2IO):
    ...
    def kill(self) -> None:
        try:
            ...
            sys.stderr.write(f"ERROR killing: {e}\n")
            sys.stderr.flush()
...
def serve_proxy_io(proxy_channelX: Channel) -> None:
    ...
    log = partial(
        proxy_channelX.gateway._trace, f"serve_proxy_io:{proxy_channelX.id}"
    )
    ...
    def forward_to_sub(data: bytes) -> None:
        log(f"forward data to sub, size {len(data)}")
        sub_io.write(data)
