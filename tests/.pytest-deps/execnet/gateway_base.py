...
class ExecModel(metaclass=abc.ABCMeta):
    ...
    def __repr__(self) -> str:
        return f"<ExecModel {self.backend!r}>"
...
class Message:
    ...
    def _status(self: Message, gateway: BaseGateway) -> None:
        ...
        gateway._send(Message.CHANNEL_DATA, self.channelid, dumps_internal(d))
        gateway._send(Message.CHANNEL_CLOSE, self.channelid)

    STATUS = 0
    _types[STATUS] = ("STATUS", _status)

    def _reconfigure(self: Message, gateway: BaseGateway) -> None:
        data = loads_internal(self.data, gateway)
        assert isinstance(data, tuple)
        strconfig: tuple[bool, bool] = data
        if self.channelid == 0:
            gateway._strconfig = strconfig
        else:
            gateway._channelfactory.new(self.channelid)._strconfig = strconfig

    RECONFIGURE = 1
    _types[RECONFIGURE] = ("RECONFIGURE", _reconfigure)

    def _gateway_terminate(self: Message, gateway: BaseGateway) -> None:
        raise GatewayReceivedTerminate(gateway)

    GATEWAY_TERMINATE = 2
    _types[GATEWAY_TERMINATE] = ("GATEWAY_TERMINATE", _gateway_terminate)

    def _channel_exec(self: Message, gateway: BaseGateway) -> None:
    ...
    def _channel_data(self: Message, gateway: BaseGateway) -> None:
        gateway._channelfactory._local_receive(self.channelid, self.data)

    CHANNEL_DATA = 4
    _types[CHANNEL_DATA] = ("CHANNEL_DATA", _channel_data)

    def _channel_close(self: Message, gateway: BaseGateway) -> None:
        gateway._channelfactory._local_close(self.channelid)

    CHANNEL_CLOSE = 5
    _types[CHANNEL_CLOSE] = ("CHANNEL_CLOSE", _channel_close)

    def _channel_close_error(self: Message, gateway: BaseGateway) -> None:
        error_message = loads_internal(self.data)
    ...
    def _channel_last_message(self: Message, gateway: BaseGateway) -> None:
...
class Channel:
    ...
    def waitclose(self, timeout: float | None = None) -> None:
        ...
        if not self._receiveclosed.is_set():
            raise self.TimeoutError(f"Timeout after {timeout!r} seconds")
    ...
    def receive(self, timeout: float | None = None) -> Any:
        ...
        try:
            ...
            raise self.TimeoutError(f"no item after {timeout!r} seconds") from None
        if x is ENDMARKER:
    ...
    def _local_receive(self, id: int, data) -> None:
        ...
        try:
            ...
            try:
                ...
                self.gateway._trace(f"exception during callback: {exc}")
    ...
    def load(self, versioned: bool = False) -> Any:
        if versioned:
            ...
            if ver != DUMPFORMAT_VERSION:
                raise LoadError(f"wrong dumpformat version {ver!r}")
