...
def test_generate_seed_queries_respects_breadth_and_contains_base():
    planner = _load_planner()
    C = planner.ExplorationConfig
    cfg = C(breadth=3, depth=0, seed=1, site_hints=["arxiv.org", "*.gov"])  # enough variety
...
def test_plan_limits_count_and_depth():
    planner = _load_planner()
    C = planner.ExplorationConfig
    cfg = C(breadth=3, depth=1, seed=7, max_queries=6, site_hints=["arxiv.org", "*.edu"])
...
def test_plan_is_deterministic_with_seed_changes_order_with_different_seed():
    ...
    cfg1 = C(breadth=3, depth=1, seed=42, max_queries=8, site_hints=["arxiv.org", "*.gov", "*.edu"]) 
    cfg2 = C(breadth=3, depth=1, seed=43, max_queries=8, site_hints=["arxiv.org", "*.gov", "*.edu"]) 
...
def test_seed_dedupes_duplicate_site_hints():
    planner = _load_planner()
    C = planner.ExplorationConfig
    cfg = C(breadth=4, depth=0, seed=0, site_hints=["arxiv.org", "https://arxiv.org", "www.arxiv.org"])  # duplicates
